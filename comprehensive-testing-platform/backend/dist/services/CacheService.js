...
export class CacheService {
    ...
    defaultTTL;
    version;
    constructor(kv, defaultTTL = 3600) {
        this.kv = kv;
     ...
     * 获取压缩的缓存
     */
    async getCompressed(key, namespace) {
        try {
     ...
     * 设置缓存（带标签）
     */
    async setWithTags(key, data, tags, options = {}) {
        try {
            const cacheKey = this.generateKey(key, options.namespace);
            const ttl = options.ttl || this.defaultTTL;
            const entry = {
     ...
     * 根据标签删除缓存
     */
    async deleteByTag(tag, namespace) {
        try {
     ...
     * 获取缓存键列表
     */
    async listKeys(prefix, namespace) {
        try {
     ...
     * 预热缓存
     */
    async warmup(key, dataProvider, options = {}) {
        try {
            // 先尝试从缓存获取
