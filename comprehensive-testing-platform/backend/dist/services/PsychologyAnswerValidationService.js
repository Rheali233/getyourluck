...
export class PsychologyAnswerValidationService {
    ...
    validateAnswerData(data) {
        ...
        switch (data.testType) {
            case 'mbti':
                this.validateMbtiAnswer(data, errors, warnings, suggestions);
                break;
            case 'phq9':
                this.validatePhq9Answer(data, errors, warnings, suggestions);
                break;
            case 'eq':
                this.validateEqAnswer(data, errors, warnings, suggestions);
                break;
            case 'happiness':
                this.validateHappinessAnswer(data, errors, warnings, suggestions);
                break;
            default:
                errors.push(`不支持的测试类型: ${data.testType}`);
        }
        // 元数据验证
        if (data.metadata) {
     ...
     * 验证MBTI答题数据
     */
    validateMbtiAnswer(data, errors, warnings, suggestions) {
        // 维度验证
     ...
     * 验证PHQ-9答题数据
     */
    validatePhq9Answer(data, errors, warnings, suggestions) {
        // 维度验证
     ...
     * 验证情商答题数据
     */
    validateEqAnswer(data, errors, warnings, suggestions) {
        // 维度验证
        const validDimensions = [
     ...
     * 验证幸福指数答题数据
     */
    validateHappinessAnswer(data, errors, warnings, suggestions) {
        // 领域验证
        const validDomains = [
    ...
    formatAnswerData(data, options) {
        ...
        switch (data.testType) {
            case 'mbti':
                formatted.dimension = data["dimension"];
                formatted.preference = data["preference"];
                formatted.confidence = data["confidence"];
                break;
            case 'phq9':
                formatted.dimension = data["dimension"];
                formatted.score = data["score"];
                formatted.severity = data["severity"];
                if (data["symptomDescription"]) {
                    formatted.symptomDescription = data["symptomDescription"];
                }
                break;
            case 'eq':
                formatted.dimension = data["dimension"];
                ...
                formatted.confidence = data["confidence"];
                if (data["reflection"]) {
                    formatted.reflection = data["reflection"];
                }
                break;
            case 'happiness':
                formatted.domain = data["domain"];
                formatted.score = data["score"];
                formatted.satisfaction = data["satisfaction"];
                formatted.importance = data["importance"];
                if (data["improvement"]) {
                    formatted.improvement = data["improvement"];
                }
                break;
        }
        return formatted;
     ...
     * 格式化答题会话数据
     */
    formatAnswerSession(session, options) {
        const formatted = {
            id: session.id,
    ...
    calculateCompletionRate(answers) {
        if (answers.length === 0)
            return 0;
        const validAnswers = answers.filter(answer => answer.answerValue !== null && answer.answerValue !== undefined);
     ...
     * 导出答题数据
     */
    exportAnswerData(sessionId, testType, answers, options) {
     ...
     * 根据测试类型获取总题目数
     */
    getTotalQuestionsByType(testType) {
     ...
     * 批量清理答题数据
     */
    cleanAnswerDataBatch(data) {
        return data.map(answer => this.cleanAnswerData(answer));
    }
}
//# sourceMappingURL=PsychologyAnswerValidationService.js.map