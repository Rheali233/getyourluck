...
export class DatabaseOptimizer {
     ...
     * 创建索引
     */
    async createIndex(tableName, indexName, columns, isUnique = false) {
        try {
    ...
    async analyzeTable(tableName) {
        ...
        WHERE pgno > 1
      `).first();
            const tableSize = sizeResult?.size || 0;
            // 获取索引信息
     ...
     * 生成表优化建议
     */
    generateTableRecommendations(tableName, rowCount, indexes) {
     ...
     * 优化查询
     */
    async optimizeQuery(originalQuery, params = []) {
        ...
        if (originalQuery.toUpperCase().includes('WHERE')) {
            const whereClause = originalQuery.toUpperCase().substring(originalQuery.toUpperCase().indexOf('WHERE'));
    ...
    async getDatabaseStats() {
      ...
      `).first();
            const totalIndexes = indexesResult?.count || 0;
            // 计算优化分数
