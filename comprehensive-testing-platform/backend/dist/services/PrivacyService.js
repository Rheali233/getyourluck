...
export class PrivacyService {
    ...
    async recordUserConsent(consent) {
      ...
      `).bind(consent.sessionId, consent.consentType, consent.granted ? 1 : 0, consent.timestamp, consent.ipAddress || null, consent.userAgent || null, consent.expiresAt || null).run();
        }
        catch (error) {
     ...
     * 获取用户同意状态
     */
    async getUserConsent(sessionId, consentType) {
        try {
            const result = await this.db.prepare(`
        SELECT granted FROM user_consents 
        WHERE session_id = ? AND consent_type = ? AND (expires_at IS NULL OR expires_at > ?)
        ORDER BY timestamp DESC LIMIT 1
      `).bind(sessionId, consentType, new Date().toISOString()).first();
     ...
     * 检查GDPR合规性
     */
    async checkGdprCompliance(sessionId) {
        try {
     ...
     * 检查数据保留状态
     */
    async checkDataRetention(sessionId) {
        try {
            const result = await this.db.prepare(`
     ...
     * 匿名化用户数据
     */
    async anonymizeUserData(sessionId, options = {}) {
        try {
     ...
     * 哈希个人数据
     */
    async hashPersonalData(sessionId) {
        try {
            // 更新用户偏好设置，移除个人标识信息
            await this.db.prepare(`
     ...
     * 移除标识符
     */
    async removeIdentifiers(sessionId) {
        try {
            // 移除IP地址和用户代理信息
            await this.db.prepare(`
    ...
    async aggregateData(sessionId) {
      ...
      `).bind(sessionId).first();
        ...
        `).bind('search_activity', searchStats.search_count, searchStats.unique_queries, new Date().toISOString()).run();
            }
        }
        catch (error) {
     ...
     * 伪匿名化数据
     */
    async pseudonymizeData(sessionId) {
        try {
            // 生成伪匿名ID
     ...
     * 实现被遗忘权
     */
    async implementRightToBeForgotten(sessionId) {
        try {
            // 删除所有用户相关数据
            await this.db.prepare(`
      ...
      `).bind(sessionId, 'right_to_be_forgotten', new Date().toISOString(), 'GDPR Article 17 - Right to erasure').run();
        }
        catch (error) {
    ...
    async cleanupExpiredData() {
      ...
      `).bind(new Date().toISOString()).run();
        ...
        WHERE created_at < ?
      `).bind(new Date(Date.now() - this.config.dataRetentionDays * 24 * 60 * 60 * 1000).toISOString()).all();
     ...
     * 获取隐私报告
     */
    async getPrivacyReport(sessionId) {
        try {
            const dataRetentionStatus = await this.checkDataRetention(sessionId);
     ...
     * 获取最后数据访问时间
     */
    async getLastDataAccess(sessionId) {
        try {
            const result = await this.db.prepare(`
