...
systemRoutes.get("/health/database", async (c) => {
    try {
        ...
        throw new ModuleError("Database health check failed", ERROR_CODES.DATABASE_ERROR, 503);
    }
});
// 系统统计信息
systemRoutes.get("/stats", rateLimiter(10, 60000), // 每分钟最多10次请求
async (c) => {
    try {
        const dbService = c.get("dbService");
        ...
        throw new ModuleError("Failed to retrieve system statistics", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
// 数据库迁移状态
systemRoutes.get("/migrations", async (c) => {
    try {
        ...
        throw new ModuleError("Failed to retrieve migration status", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
// 缓存健康检查
...
systemRoutes.get("/config", async (c) => {
    try {
        ...
        throw new ModuleError("Failed to retrieve system configuration", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
// 清理过期数据（管理员功能）
systemRoutes.post("/cleanup", rateLimiter(1, 3600000), // 每小时最多1次
async (c) => {
    try {
        const dbService = c.get("dbService");
        ...
        throw new ModuleError("Failed to cleanup expired data", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
// 创建数据库备份（管理员功能）
systemRoutes.post("/backup", rateLimiter(1, 3600000), // 每小时最多1次
async (c) => {
    try {
        const dbService = c.get("dbService");
        ...
        throw new ModuleError("Failed to create database backup", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
// 系统信息概览
systemRoutes.get("/info", async (c) => {
    try {
        ...
        throw new ModuleError("Failed to retrieve system information", ERROR_CODES.DATABASE_ERROR, 500);
    }
});
export { systemRoutes };
