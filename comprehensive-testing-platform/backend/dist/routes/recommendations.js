...
recommendationsRoutes.get('/', async (c) => {
    try {
        ...
        // 生成推荐逻辑
        const recommendations = await generateRecommendations(popularModules, userPreferences, searchHistory, parseInt(limit) || 6);
        return c.json({
            success: true,
 ...
 * 生成推荐逻辑
 */
async function generateRecommendations(popularModules, userPreferences, searchHistory, limit) {
 ...
 * 计算相关性分数
 */
function calculateRelevanceScore(module, userPreferences, searchHistory) {
    let score = 0.5; // 基础分数
    // 基于用户偏好调整分数
    if (userPreferences) {
    ...
    if (searchHistory.length > 0) {
        const relevantSearches = searchHistory.filter(search => search.query.toLowerCase().includes(module.name.toLowerCase()) ||
    ...
    // 基于模块热度调整分数
    if (module.totalUsers > 1000)
        score += 0.1;
    if (module.averageRating > 4.0)
        score += 0.05;
 ...
 * 获取推荐原因（中文）
 */
function getRecommendationReason(score, module) {
    if (score >= 0.8)
        return '高度匹配您的兴趣';
    if (score >= 0.6)
        return '基于您的偏好推荐';
 ...
 * 获取推荐原因（英文）
 */
function getRecommendationReasonEn(score, module) {
    if (score >= 0.8)
        return 'Highly matches your interests';
    if (score >= 0.6)
        return 'Recommended based on your preferences';
    if (score >= 0.4)
        return 'Popular test recommendation';
