...
import { 
  PsychologyQuestionBankModel,
  type PsychologyQuestionCategoryData,
  type PsychologyQuestionData,
  type PsychologyQuestionOptionData,
  type PsychologyQuestionConfigData,
  type PsychologyQuestionVersionData,
  type CreatePsychologyQuestionCategoryData,
  type CreatePsychologyQuestionData,
  type CreatePsychologyQuestionOptionData,
  type CreatePsychologyQuestionConfigData,
  type CreatePsychologyQuestionVersionData
} from "../models";
...
export class PsychologyQuestionBankService {
  private questionBankModel: PsychologyQuestionBankModel;
    ...
    try {
      ...
      if (existingCategory) {
        throw new ModuleError(
          `Question category with code '${data.code}' already exists`,
      ...
      throw new ModuleError(
        `Failed to create question category: ${error instanceof Error ? error.message : 'Unknown error'}`,
   ...
   * 根据代码获取题库分类
   */
  async getCategoryByCode(code: string): Promise<PsychologyQuestionCategoryData | null> {
    try {
      // 尝试从缓存获取
      const cacheKey = `psychology:category:${code}`;
      const cached = await this.getCache<PsychologyQuestionCategoryData>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get question category: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getAllActiveCategories(): Promise<PsychologyQuestionCategoryData[]> {
    try {
      ...
      throw new ModuleError(
        `Failed to get active categories: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async createQuestion(data: CreatePsychologyQuestionData): Promise<string> {
    try {
      ...
      if (!category) {
        throw new ModuleError(
          `Question category not found: ${data.categoryId}`,
  ...
  async getQuestionsByCategory(categoryId: string): Promise<PsychologyQuestionData[]> {
    try {
      ...
      const cached = await this.getCache<PsychologyQuestionData[]>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get questions by category: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getQuestionsByDimension(dimension: string): Promise<PsychologyQuestionData[]> {
    try {
      ...
      throw new ModuleError(
        `Failed to get questions by dimension: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getQuestionsByParams(params: QuestionBankQueryParams): Promise<PsychologyQuestionData[]> {
    try {
      ...
      throw new ModuleError(
        `Failed to get questions by params: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async createQuestionOption(data: CreatePsychologyQuestionOptionData): Promise<string> {
    try {
      ...
      throw new ModuleError(
        `Failed to create question option: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getOptionsByQuestion(questionId: string): Promise<PsychologyQuestionOptionData[]> {
    try {
      ...
      const cached = await this.getCache<PsychologyQuestionOptionData[]>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get options by question: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async createConfig(data: CreatePsychologyQuestionConfigData): Promise<string> {
    try {
      ...
      throw new ModuleError(
        `Failed to create question config: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getConfigsByCategory(categoryId: string): Promise<PsychologyQuestionConfigData[]> {
    try {
      ...
      const cached = await this.getCache<PsychologyQuestionConfigData[]>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get configs by category: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async createVersion(data: CreatePsychologyQuestionVersionData): Promise<string> {
    try {
      ...
      throw new ModuleError(
        `Failed to create question version: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getActiveVersion(categoryId: string): Promise<PsychologyQuestionVersionData | null> {
    try {
      ...
      const cached = await this.getCache<PsychologyQuestionVersionData>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get active version: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
  ...
  async getQuestionBankStats(): Promise<QuestionBankStats> {
    try {
      ...
      const cached = await this.getCache<QuestionBankStats>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      throw new ModuleError(
        `Failed to get question bank stats: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ERROR_CODES.DATABASE_ERROR,
        500
      );
    }
   ...
   * 设置缓存
   */
  private async setCache<T>(key: string, data: T, ttl: number = this.cacheTTL): Promise<void> {
    try {
   ...
   * 清除分类相关缓存
   */
  private async clearCategoryCache(categoryCode: string): Promise<void> {
    try {
      const keys = [
   ...
   * 清除题目相关缓存
   */
  private async clearQuestionCache(categoryId: string): Promise<void> {
    try {
      const keys = [
   ...
   * 清除配置相关缓存
   */
  private async clearConfigCache(categoryId: string): Promise<void> {
    try {
      const keys = [
   ...
   * 清除版本相关缓存
   */
  private async clearVersionCache(categoryId: string): Promise<void> {
    try {
      const keys = [
