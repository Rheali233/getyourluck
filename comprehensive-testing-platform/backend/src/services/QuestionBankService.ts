...
import { 
  ...
  type QuestionConfigData,
  type QuestionVersionData,
  type CreateQuestionCategoryData,
...
export class QuestionBankService {
  ...
  constructor(env: Env) {
    this.questionBankModel = new QuestionBankModel(env);
    this.cache = env.KV!;
  }
   ...
   * 根据代码获取题库分类
   */
  async getCategoryByCode(code: string): Promise<QuestionCategoryData | null> {
    try {
      // 尝试从缓存获取
      const cacheKey = `psychology:category:${code}`;
      const cached = await this.getCache<QuestionCategoryData>(cacheKey);
      if (cached) {
        return cached;
      }
  ...
  async getQuestionsByCategory(categoryId: string): Promise<QuestionData[]> {
    try {
      ...
      const cached = await this.getCache<QuestionData[]>(cacheKey);
      if (cached) {
        return cached;
      }
   ...
   * 根据查询参数获取题目列表
   */
  async getQuestionsByParams(params: QuestionBankQueryParams): Promise<QuestionData[]> {
    try {
  ...
  async getOptionsByQuestion(questionId: string): Promise<QuestionOptionData[]> {
    try {
      ...
      const cached = await this.getCache<QuestionOptionData[]>(cacheKey);
      if (cached) {
        return cached;
      }
  ...
  async getConfigsByCategory(categoryId: string): Promise<QuestionConfigData[]> {
    try {
      ...
      const cached = await this.getCache<QuestionConfigData[]>(cacheKey);
      if (cached) {
        return cached;
      }
  ...
  async getActiveVersion(categoryId: string): Promise<QuestionVersionData | null> {
    try {
      ...
      const cached = await this.getCache<QuestionVersionData>(cacheKey);
      if (cached) {
        return cached;
      }
